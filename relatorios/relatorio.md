...
- Conclusão: sim, a arquitetura com contexto custou mais (parte 4) — mas o ganho não é só "código mais bonito". É definir regras técnicas e de negócio *antes* de sair rodando prompt, e rodar em partes (uma task por vez) em vez de uma tacada única — isso deixa fácil revisar cada pedaço, achar o que saiu errado e ajustar antes de seguir pra próxima etapa. Numa feature isolada de fim de semana, um prompt seco resolve. Num projeto maior, com várias pessoas e módulos que precisam conversar entre si, essa arquitetura é a mais indicada — por controle de qualidade, rastreabilidade (cada task = uma chamada revisável) e consistência entre as partes.
